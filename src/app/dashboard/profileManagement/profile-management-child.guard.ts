...
export class ProfileManagementChildGuard implements CanActivateChild {
  ...
  async canActivateChild(
    ...
    let profile: any;
    try { profile = await this._managementService.getProfileDetail(userInfo); } catch (err) { console.log(err); }
    if (!profile) {
      this._router.navigate(['/']);
      return false;
    }
    ...
    switch (url) {
      ...
      case 'reviews-ratings':
        ...
        if (role === 'u') {
          return true;
        } else if (profile.isVipAffiliateUser) {
          return true;
        } else if (profile.plan && profile.plan.name !== 'Basic') {
          return true;
        }
        break;
      ...
      case 'my-product':
        ...
        if (role === 'c') {
          if (profile.isVipAffiliateUser) {
            return true;
          } else if (profile.plan && profile.plan.ListProductsOption) {
            return true;
          }
        }
        break;
      ...
      case 'add-professionals':
        ...
        if (role === 'c') {
          if (profile.isVipAffiliateUser) { return true; } else if (profile.plan && profile.plan.ListOfProviders) { return true; }
        }
        break;
      ...
      case 'my-amenities':
        ...
        if (role === 'c') {
          if (profile.isVipAffiliateUser) { return true; } else if (profile.plan && profile.plan.ListAmenities) { return true; }
        }
        break;
    ...
    if (role === 'p') {
      this._router.navigate(['/dashboard/profilemanagement/partner-profile']);
    } else {
